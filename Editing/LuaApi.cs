...
namespace NoCodeMotion.Editing
{
    ...
    public static class LuaApi
    {
        ...
        public static readonly List<LuaSymbol> HardwareList = new List<LuaSymbol>
        {
            ...
            F("PointMove", "PointMove(\"点位表.点位名\")", "点位移动：按点位表各轴槽把轴开到该点位并等待到位（未填位置的槽跳过），返回驱动的轴数"),
            F("PointModify", "PointModify(\"点位表.点位名\", 轴槽1~4, 目标位置, 速度)", "点位修改：改点位某个轴槽的目标位置（速度填 0 表示不改），真写回工程并保存"),
            F("PointTeach", "PointTeach(\"点位表.点位名\")", "点位示教：用各轴当前位置更新该点位的目标位置（真写回工程并保存）"),
