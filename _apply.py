...
def find_close(text, open_start):
    ...
    while i<len(text):
        ...
        if om:
            egt=text.find('>', lt)
            if egt!=-1 and text[egt-1]=='/': i=egt+1; continue
...
def remove_dock_right(elem): return elem.replace(' DockPanel.Dock="Right"', '', 1)
...
def transform_dockpanels(s, hints):
    ...
    while True:
        ...
        if close==-1:
            gt=s.find('>', dp)
            if gt!=-1:
                out.append(s[dp:gt+1]); last=gt+1
            else:
                last=dp+1
            continue
...
def transform_chips(s, hints):
    ...
    while True:
        ...
        if not im:
            out.append(block); last=close; continue
        estart=im.start()
...
os.makedirs('D:/wqz/code/NoCodeMotion/_out', exist_ok=True)
for f in FILES:
    s, enc, crlf = read_text(f)
    s2, dc, dfound = transform_dockpanels(s, HINTS)
    s3, cc, cfound = transform_chips(s2, HINTS)
    outp='D:/wqz/code/NoCodeMotion/_out/'+f.split('/')[-1]
    write_text(outp, s3, enc, crlf)
    print("%-15s dock=%2d/%2d chip=%2d/%2d opens=%d closes=%d"%(f.split('/')[-1],dc,dfound,cc,cfound,s3.count('<local:Field'),s3.count('</local:Field>')))
