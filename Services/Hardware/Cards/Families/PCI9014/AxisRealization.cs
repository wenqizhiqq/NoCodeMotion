...
//   源: MotionCardRes/PCI9014/AxisRealization.cs
...
namespace Samsun.Domain.MotionCard.Common.PCI9014
{
    public class AxisRealization : IAxis
    {
        ...
        {
            return PCI9014SDK.Instance.SetCardAxisPos(AxisID, cntr_on, Pos);
        }
        ...
        public int AxisAlarm
        {
            ...
            {
                uint level = 0;
                var res = PCI9014SDK.Instance.GetCardDIBit(AxisWhichCardNo, (uint)AxisID, ref level);
                return 0; // (int)level;
            }
            set
            {
        ...
        {
            uint status = 0;
            var res = PCI9014SDK.Instance.GetAxisCardIOStatus(AxisID, ref status);
            if (res == 0)
                return (int)status;
            else
                return res;
        ...
        {
            var res = PCI9014SDK.Instance.GetErrorInfo(ErrNum);
            return res;
        }
        ...
        {
            return PCI9014SDK.Instance.SetCardAxisHomeConfig(hpm.Axis, hpm.HomeMode, hpm.OrgLevel, hpm.EZlevel);
        }
        ...
        {
            //SetCardAxisHomeProfile();已经设置过
            if (PCI9014SDK.Instance.homeConfigArray == null || PCI9014SDK.Instance.homeConfigArray.Count() < (4 * (hpm.CardNo + 1))) PCI9014SDK.Instance.homeConfigArray = new HomeParameModel[4 * (hpm.CardNo + 1)];
            PCI9014SDK.Instance.homeConfigArray[hpm.Axis] = hpm;
            return 0;
        }
        ...
        {
            return PCI9014SDK.Instance.SetCardAxisELLevel(lpm.Axis, lpm.ActiveLevel);
        }
        ...
        {
            return PCI9014SDK.Instance.SetCardAxisErrorStopMode(axis, stop_mode);
        }
        ...
        {
            return PCI9014SDK.Instance.SetCardAxisPos(axis, 0, pos);
        }
        ...
        public int GetCardAxisInitVel(/*int CardNo,int Axis, */ref int pSpeed)
        {
            ...
            double pspeed = 0;
            int result = PCI9014SDK.Instance.GetCardAxisCurrentSpeed(AxisID, ref pspeed);
            pSpeed = (int)pspeed;
            return result;
        }
        ...
        public double GetCardAxisCurrentPosition(int cntr_no/*, int Axis*/)
        {
            ...
            //所以写成下面这样
            if (PCI9014SDK.Instance.GetCardAxisPos(AxisID, cntr_no, ref pos) == 0)
            {
                return pos;
            }
            return -1;
        ...
        public int GetCardAxisCurrentState()
        {
            ...
            uint u = 0;
            int axis = AxisID;
            var res = PCI9014SDK.Instance.GetCardAxisMotionStatus(axis, ref u);
            if (res == 0)
                return (int)u;
            else
                return -1;
        ...
        {
            return PCI9014SDK.Instance.SetCardAxisAlarm(axis, enable, active_level);
        }
        ...
        {
            return PCI9014SDK.Instance.SetCarsAxisSProfile(mpm.Axis, mpm.MinVel, mpm.MaxVel, mpm.TaccVel, mpm.TdecVel, mpm.JerkPercent);
        }
        ...
        {
            return PCI9014SDK.Instance.SetCarsAxisTProfile(mpm.Axis, mpm.MinVel, mpm.MaxVel, mpm.TaccVel, mpm.TdecVel);
        }
        ...
        {
            if (PCI9014SDK.Instance.homeConfigArray[AxisID].HomeMode == 4 || PCI9014SDK.Instance.homeConfigArray[AxisID].HomeMode == 2)
            {

                PCI9014SDK.Instance.CardAxisHomeMove(AxisID, 0);
                PCI9014SDK.Instance.CardAxisPMove(AxisID, (int)PCI9014SDK.Instance.homeConfigArray[AxisID].HomeTrigNegMoveDir, 0, 1);
                ...
                {
                    Axis = PCI9014SDK.Instance.homeConfigArray[AxisID].Axis,
                    CardNo = PCI9014SDK.Instance.homeConfigArray[AxisID].CardNo,
                    MinVel = 10000,
                    MaxVel = 11000,
                    TaccVel = 0.1,
                    TdecVel = 0.1
                });
                return PCI9014SDK.Instance.CardAxisHomeMove(AxisID, 0);
            }
            else
                return PCI9014SDK.Instance.CardAxisHomeMove(AxisID, 0);
        ...
        {
            return PCI9014SDK.Instance.CardAxisPMove(mpm.Axis, (int)mpm.Dist, mpm.PosiMode, mpm.RunMode);
        }
        ...
        {
            return PCI9014SDK.Instance.CardAxisVMove(mpm.Axis, mpm.Dir, mpm.RunMode);
        }
        ...
        {
            return PCI9014SDK.Instance.CardAxisStop(axis, emg_stop);
        }
        ...
        public int CardAxisLineUnit(LineInterpolateparamModel line)
        {
            ...
            return PCI9014SDK.Instance.CardAxisLineMulticoor(line.CardNo, line.Crd, line.Axis.Length, AxisNums, Target_Pos, line.PosiMode);
        ...
        {
            return PCI9014SDK.Instance.SetCardAxisPos(axis, 1, encoder_value);
            //throw new NotImplementedException();
        }
        ...
        public int SetStatusParameter(int axis, bool f, AxisStatusParameterEnum kind)
        {
            ...
            if (kind.ToString() == "正限位" || kind.ToString() == "负限位")
            {
                if (f == true)
                {
                    res = PCI9014SDK.Instance.SetCardAxisELLevel(axis, 1);
                }
                else
                {
                    res = PCI9014SDK.Instance.SetCardAxisELLevel(axis, 0);
                }
            }
            if (kind.ToString() == "原点位")
            {
                if (f == true)
                {
                    res = PCI9014SDK.Instance.SetCardAxisHomeConfig(axis, 0, 1, 0);
                }
                else
                {
                    res = PCI9014SDK.Instance.SetCardAxisHomeConfig(axis, 0, 0, 0);
                }
            }
        ...
        public int CardAxisGetHomeResult(int CardNo, int axis, ref ushort status)
        {
            //throw new NotImplementedException();
            uint pstatus = 1;
            var res = PCI9014SDK.Instance.GetCardAxisMotionStatus(axis, ref pstatus);
            if (pstatus == 1)
                status = 0;
            else
                status = 1;
        ...
        public double GetCardAxisCurrentSpeed(int CardNo, int axis)
        {
            ...
            var res = PCI9014SDK.Instance.GetCardAxisCurrentSpeed(axis, ref currentSpeed);
