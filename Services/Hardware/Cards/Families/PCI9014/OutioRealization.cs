...
//   源: MotionCardRes/PCI9014/OutioRealization.cs
...
namespace Samsun.Domain.MotionCard.Common.PCI9014
{
    public class OutioRealization : IIOOutPut
    {
        ...
        /// 硬件卡拥有的输出IO的数量
        /// </summary>
        public ushort OutIONums { get; set; } = 16;
        ...
        {
            return PCI9014SDK.Instance.SetCardDOBit(CardNo, (uint)bitno, (uint)on_off);
        }
        ...
        {
            return PCI9014SDK.Instance.SetCardDO(CardNo, outport_val);
        }
        ...
        {
            uint pData = 0;
            PCI9014SDK.Instance.GetCardDO(CardNo, ref pData);
            return pData;
        }

        /// <summary>
        /// 读取指定控制卡的某个输出端口的电平(9014卡不支持这个方法,调用会抛异常NotImplementedException)
        /// </summary>
        /// <param name="CardNo">控制卡卡号</param>
        ...
        public int GetCardPortNoOutState(int CardNo, int bitno)
        {
            ...
            uint pData = 0;
            PCI9014SDK.Instance.GetCardDO(CardNo, ref pData);
            var s = Convert.ToString(pData, 2).PadLeft(32, '0');
            s = new string(s.ToCharArray().Reverse().ToArray());
            //return s[bitno];
            //int data = int.Parse(s[bitno].ToString());
            return int.Parse(s[bitno].ToString());
        }
        ...
        {
            return PCI9014SDK.Instance.SetCardDO(CardNo, (uint)data);
        }
        ...
        /// <returns>  正常返回 0；  出现错误时返回非 0 值；</returns>
        public int SetCardDOBitState(int CardNo, int BitNo, int data)
        {
            return PCI9014SDK.Instance.SetCardDOBit(CardNo, (uint)BitNo, (uint)data);
        }
        ...
        {
            uint pdata = 0;
            int result = PCI9014SDK.Instance.GetCardDO(CardNo, ref pdata);
            pData = (int)pdata;
            return result;
        }
        ...
        ///// 读取指定输出端口的状态
        ///// </summary>
        //public short GetCardReadOutBit()
        //{
        //    return 0;
        //}
    }
