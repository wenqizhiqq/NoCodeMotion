...
//   源: MotionCardRes/RT3316/rt3316SDK.cs
//   改动: 仅行尾归一化为 LF、加 #nullable disable；命名空间与逻辑保持原样。
// ────────────────────────────────────────────────────────────────
#nullable disable
using System.Collections.Generic;
using System.Threading;
using RtMotionApiNET;

namespace Samsun.Domain.MotionCard.Common.RT3316
{
    public class RT3316SDK
    {
        public RtMotion rtApi;

        bool m_bEthernetErr = false;
        bool m_bEtherCATErr = false;


        public RT3316SDK()
        {
            rtApi = new RtMotion();
            rtApi.EtherCATErrorEvent += EventOccur;       // 注册 EtherCAT通讯错误事件
            rtApi.EthernetErrorEvent += EventOccur;       // 注册 Ethernet通讯错误事件
        }

        private int EventOccur(ulong eventSender, int eventType)
        {
            var eventString = "";
            if (eventSender == (ulong)EventSender.EventSender_Controller)
            {
                // Ethernet 通讯发生错误事件
                if (eventType == (int)EventType.EthernetDisconnect)
                {
                    m_bEthernetErr = true;
                    eventString = "TCP 通讯发生错误!\r\n";
                }
                // EtherCAT 通讯发生错误事件
                else if (eventType == (int)EventType.EtherCATDisconnect)
                {
                    m_bEtherCATErr = true;
                    eventString = "EtherCAT 通讯发生错误!\r\n";
                }
            }
        ...
        private static RT3316SDK _instance;
        public static RT3316SDK Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new RT3316SDK();
                return RT3316SDK._instance;
            }
            set { RT3316SDK._instance = value; }
        }
        ...
        /// 初始化 RT3316卡控制卡
        /// </summary>
        /// <param name="card_id">有*existCards 个元素的数组，包含查找到的卡 的 ID 号，建议使用含 有 16 个元素的 I32 类型数组作为该参数 </param>
        ...
        public int InitCard(ref int[] card_id, ref int card_count)
        {
            ...
            try
            {
                rtApi.openCommEthernetTCP("192.168.0.10", 7);
                card_id[0] = 3316;
                card_count = 1;
                return 0;
            }
        ...
        public int SetCardAxisPos(int axis, int cntr_on, int Pos)
        {
            ...
            //}
            rtApi.setCmdPosition(axis, Pos);
            return 0;
           
            //注意，如果想编码器位置清0，这个卡跟其它卡不同，见下面说明：
            /*
            不论轴伺服电机是一般相对式编码器时还是绝对式编码器, 机台上电后要标定源点或实现坐标偏移可透过下面
            步骤:

            相对式编码器伺服
                a.相对式编码器伺服因为无断电位置记忆功能, 因此每次上电后需先归机械原点(请参考 4.e 回原点运
                动的实现).
                b.开机后软件运行回零完成后, 调用 rt_setCmdPosition()设定初始轴初始坐标. (此时控制器内部会产
                生 Offset 偏移值)
                c.完成

            绝对式编码器伺服
                a.绝对式编码器伺服有断电位置记忆功能, 因此上电后理论上不需执行回零动作.
                b.如需要将绝对式编码器清除为 0 或设定为指定位置, 可将滑台移动到原点位置后,
                c.将原点模式设置为 HomeMode35(调用 rt_setHomeMode()), 并将设置指定位置(透过
                rt_setHomeOffset()) 后, 再执行 rt_startHome() 就可以完成零点设
            */
        }
        ...
        /// <returns>正常返回 0；  出现错误时返回非 0 值；</returns>
        public int CardAxisPMove(int axis, double dist, int dist_mode, int vel_mode)
        {
            var res = -1;
            try
            {
                ...
                {
                    //相对
                    rtApi.move(axis, MotionFlag.Relative, dist, 0);
                }
                else
                {
                    //绝对
                    rtApi.move(axis, MotionFlag.None, dist, 0);
                }
            }
            catch
            {
        ...
        axis 轴号, 0, 1 ,2...
        flag
            辅助标志位, 可透过 " | " 位运算符设定包含以下一个或多个标志的位参数
            RT_MF_VELOCITY 0x01 运动将使用 velocity 指定的速度参数而不是默认速度
            RT_MF_RELATIVE 0x02 运动将使用相对坐标(默认绝对)
        dir
            运动方向
            RT_POSITIVE_DIRECTION 1 正向移动
            RT_NEGATIVE_DIRECTION 0 负向移动
        velocity 
            如果指定了 RT_MF_VELOCITY 标志，则使用此 velocity 速度参数值
            */
            //rtApi.jog()
