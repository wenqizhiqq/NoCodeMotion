...
//   源: MotionCardRes/DigitalTwinCard/VirtualCardSDK.cs
//   改动: 仅行尾归一化为 LF、加 #nullable disable；命名空间与逻辑保持原样。
// ────────────────────────────────────────────────────────────────
#nullable disable
using MotionCardRes.DigitalTwinCard;
using System;
...
namespace Samsun.Domain.MotionCard.Common.DigitalTwinCard
{
    /// <summary>
    /// 虚拟运动卡SDK
    /// </summary>
    public class VirtualMotionCardSDK
    {
        private mappingAxisIODataStruct axisIOStatusBuffer;
        public bool IsInit { get; set; }

        private static readonly Lazy<VirtualMotionCardSDK> lazy = new Lazy<VirtualMotionCardSDK>(() => new VirtualMotionCardSDK());
        public static VirtualMotionCardSDK Instance { get { return lazy.Value; } }

        public enum inputIONameEnum
        {
            IN0, IN1, IN2, IN3, IN4, IN5, IN6, IN7, IN8, IN9, IN10, IN11, IN12, IN13, IN14_LTC0,
            IN15_LTC1, EL0P, EL1P, EL2P, EL3P, EL4P, EL5P, EL6P, EL7P, EL0N, EL1N, EL2N, EL3N, EL4N, EL5N, EL6N, EL7N,
            ORG0, ORG1, ORG2, ORG3, ORG4, ORG5, ORG6, ORG7, ALM0, ALM1, ALM2, ALM3, ALM4, ALM5, ALM6, ALM7,
            RDY0, RDY1, RDY2, RDY3, RDY4, RDY5, RDY6, RDY7, INP0, INP1, INP2, INP3, INP4, INP5, INP6, INP7
        };

        public enum outputIONameEnum
        {
            OUT0, OUT1, OUT2, OUT3, OUT4, OUT5, OUT6, OUT7, OUT8, OUT9, OUT10, OUT11, OUT12_CMP0, OUT13_CMP1, OUT14_CMP2,
            OUT15_CMP3, SEVON0, SEVON1, SEVON2, SEVON3, SEVON4, SEVON5, SEVON6, SEVON7, ERC0, ERC1, ERC2, ERC3, ERC4, ERC5, ERC6, ERC7
        };


        /// <summary>
        /// 主板带的输入IO的位状态缓存。key为io编号
        /// </summary>
        public Dictionary<ushort, VitualCardInputIOStruct> MainBordInputIOBitBuffer = new Dictionary<ushort, VitualCardInputIOStruct>();

        /// <summary>
        /// 主板带的输出IO的位状态缓存
        /// </summary>
        public Dictionary<ushort, VitualCardOutputIOStruct> MainBordOutputIOBitBuffer = new Dictionary<ushort, VitualCardOutputIOStruct>();


        /// <summary>
        /// 扩展模块带的输入IO的位状态缓存
        /// </summary>
        public Dictionary<string, VitualCardExtendInputIOStruct> ExtendInputIOBitBuffer = new Dictionary<string, VitualCardExtendInputIOStruct>();

        /// <summary>
        /// 扩展模块带的输出IO的位状态缓存
        /// </summary>
        public Dictionary<string, VitualCardExtendOutputIOStruct> ExtendOutputIOBitBuffer = new Dictionary<string, VitualCardExtendOutputIOStruct>();


        public struct VitualCardExtendInputIOStruct
        {
            /// <summary>
            /// 输入口名字
            /// </summary>
        ...
        public struct VitualCardExtendOutputIOStruct
        {
            /// <summary>
            /// 输入口名字
            /// </summary>
        ...
        public struct VitualCardInputIOStruct
        {
            /// <summary>
            /// 输入口名字
            /// </summary>
        ...
        public struct VitualCardOutputIOStruct
        {
            /// <summary>
            /// 输出口名字
            /// </summary>
        ...
        public void SaveSACMotionDebugConfig(string filename = "MappingAxisIOConfigData")
        {
            ...
            try
            {
                ...
                var str1 = Newtonsoft.Json.JsonConvert.SerializeObject(axisIOStatusBuffer);
        ...
        /// <summary>
        /// 载入轴映射参数 异常：ArgumentException
        /// </summary>
        /// <param name="filename"></param>
        /// <returns></returns>
        public void LoadSACMotionDebugConfig(string filename = "MappingAxisIOConfigData")
        {
            string moduleDir = AppDomain.CurrentDomain.BaseDirectory;
            try
            {
            ...
            {
                throw new ArgumentException("读参数配置文件，反序列化时失败，详细信息：" + ex.Message);
            }
            ...
            try
            {
                ...
                {
                    axisIOStatusBuffer = new mappingAxisIODataStruct();
                }
                else
                {
                    axisIOStatusBuffer = Newtonsoft.Json.JsonConvert.DeserializeObject<mappingAxisIODataStruct>(str1);
                }
            }
            catch (Exception ex)
            {
                throw new ArgumentException("读调试参数配置文件，反序列化时失败，详细信息：" + ex.Message);
            }
        }


        public VirtualMotionCardSDK()
        {
          
      
            var inputNameAry = Enum.GetNames(typeof(inputIONameEnum));
            var outputNameAry = Enum.GetNames(typeof(outputIONameEnum));
            for (int i = 0; i < 64; i++)
            {
                MainBordInputIOBitBuffer.Add((ushort)i, new VitualCardInputIOStruct()
                {
                    Name = inputNameAry[i],
                    InputPortNo = i,
                    BitValue = true
            ...
            for (int i = 0; i < 32; i++)
            {
                MainBordOutputIOBitBuffer.Add((ushort)i, new VitualCardOutputIOStruct()
                {
                    Name = outputNameAry[i],
                    OutputPortNo = i,
                    BitValue = true
                });

                List<double> motionParameter = new List<double>();
                for (int j = 0; j < 5; j++)
                {
                    motionParameter.Add(0);
                }
                axisParameterBuffer.Add(i, motionParameter);
                ...
                axisStatusParameter.Add(i, status);

                List<int> axisSpacingList = new List<int>();
                for (int j = 0; j < 3; j++)
                {
                    axisSpacingList.Add(0);
                }
                axisSpacing.Add(i, axisSpacingList);

                isRun[i] = false;
            }
            for (int i = 0; i < 32; i++)
            {
                axisStatusParameterStr[i] = "00000000000000000001000000000000";// axisStatusParameter[i][(int)AxisStatusEnum.报警].ToString() + axisStatusParameter[i][(int)AxisStatusEnum.正限位].ToString() + axisStatusParameter[i][(int)AxisStatusEnum.负限位].ToString() + axisStatusParameter[i][(int)AxisStatusEnum.急停].ToString() + axisStatusParameter[i][(int)AxisStatusEnum.原点位].ToString() + 0 + 0 + 0 + 0 + 0;
            }
            ...
            for (int k = 0; k < 16; k++)
            {
                for (int i = 0; i < 256; i++)
                {
                    ExtendInputIOBitBuffer.Add($"{k},{i}",
                        new VitualCardExtendInputIOStruct()
                        {
                            NodeNo = k,
                            InputPortNo = i,
                            BitValue = true,
                    ...
                    ExtendOutputIOBitBuffer.Add($"{k},{i}",
                       new VitualCardExtendOutputIOStruct()
                       {
                           NodeNo = k,
                           OutputPortNo = i,
                           BitValue = true,
            ...
            //ExtendInputIOBitBuffer
            //ExtendOutputIOBitBuffer
            ...
            Task.Factory.StartNew(() =>
            {
                while (true)
                {
                    if (!ModubsComm.Instance.isLinkServer)
                    {
                        Thread.Sleep(5);
                        continue;
                    }
                    ModubsComm.Instance.Read4AreaData();
                    Thread.Sleep(100);
                }
        ...
        public string GetErrorInfo(int ErrNum)
        {
            ...
            Dictionary<int, string> dic = new Dictionary<int, string>()
            {
                ...
                {6,"ERR_CONTILINE" },
                {8,"ERR_CANNOT_CONNECTETH" },
                {9,"MOTION_ERR_HANDLEERR" },
                {10,"ERR_SENDERR" },
                {12,"ERR_FIRMWAREERR" },
                {14,"ERR_FIRMWAR_MISMATCH" },
                {20,"ERR_FIRMWARE_INVALID_PARA_ERR" },
                {22,"ERR_FIRMWARE_STATE_ERR" },
                {24,"ERR_FIRMWARE_CARD_NOT_SUPPORT" }
            };
             ...
             *   9  MOTION_ERR_HANDLEERR   卡号错误，可能由于函数参数中的卡号或轴号超出范围产生，比如有两张卡分别为 0 号和1 号卡，当轴号为 8 时根据计算应该为 2 号卡，此时就会产生错误。
             *   10 ERR_SENDERR   数据传输错误，请检查 PCI 槽位是否松动
        ...
        {
            //由磁盘文件初始化axisStatusDataModbusCoilIdx
            try
            {
                LoadSACMotionDebugConfig();  //载入轴io状态数据的文件，位于bin目录下的MappingAxisIOConfigData文件
            }
            catch
            {
                for (int i = 0; i < 64; i++)
                {
                    for (int k = 0; k < 32; k++)
                    {
                        axisStatusDataModbusCoilIdx[i, k] = -1; //负数表示没有指定modbus继电器区索引,在配置窗口AxisIODataMappingForm上显示为空
                    }
                }
            ...
            //连接仿真器的modbus slave端
            if (!ModubsComm.Instance.open()) return 0;


            return 1; // LTDMC.dmc_board_init();
        }
        ...
        public int CardBoardReset()
        {
            ...
            return 0; // LTDMC.dmc_board_reset();
        }
        ...
        {
            return 0; // LTDMC.dmc_soft_reset(CardNo);
        }
        ...
        {
            if (!ModubsComm.Instance.close()) return -1;
            return 0; // LTDMC.dmc_board_close();
        }
        ...
        {
            CardNum = 1;
            CardTypeList[0] = 555000;
            CardIdList[0] = 0;
            return 0; // LTDMC.dmc_get_CardInfList(ref CardNum, CardTypeList, CardIdList);
        }
        ...
        {
            return 0; // LTDMC.dmc_set_vector_profile_multicoor(cardNo, crd, minVel, maxVel, tacc, 0, 0);
        }
        ...
        {
            CardVersion = 2021;
            return 0; // LTDMC.dmc_get_card_version((ushort)CardNo, ref CardVersion);
        }
        ...
        public int GetCardSoftVersion(int CardNo, ref UInt32 FirmID, ref UInt32 SubFirmID)
        {
            FirmID = 1111;
            SubFirmID = 0000;
            return 0; // LTDMC.dmc_get_card_soft_version((ushort)CardNo, ref FirmID, ref SubFirmID);
        }
        ...
        {
            LibVer = 3;
            return 0; // LTDMC.dmc_get_card_lib_version(ref LibVer);
        }
        ...
        {
            TotalAxis =64;
            return 0;// LTDMC.dmc_get_total_axes((ushort)CardNo, ref TotalAxis);
        }
        ...
        /// <para>2）可以在 Motion3000 软件中“参数设置”界面下，将各轴参数设置好，然后点击“参数文件操作”-“读取”将参数文件保存。最后在编写程序时，使用函数dmc_download_configfile 将参数文件下载 </para>
        /// </summary>
        /// <param name="CardNo">卡号</param>
        ...
        {
            return 0; // LTDMC.dmc_download_configfile((ushort)CardNo, FileName);
        }
        ...
        {
            return 0; // LTDMC.dmc_download_firmware((ushort)CardNo, FileName);
        }
        ...
        {
            return 0; // LTDMC.dmc_set_pulse_outmode((ushort)CardNo, (ushort)axis, (ushort)outmode);
        }
        ...
        {
            outmode = 0;
            return 0; // LTDMC.dmc_get_pulse_outmode((ushort)CardNo, (ushort)axis, ref outmode);
        }
        #endregion
        ...
        {
            return 0; // LTDMC.dmc_set_home_pin_logic((ushort)CardNo, (ushort)axis, (ushort)org_logic, filter);
        }
        ...
        {
            return 0; // LTDMC.dmc_get_home_pin_logic((ushort)CardNo, (ushort)axis, ref org_logic, ref filter);
        }
        ...
        {
            return 0; // LTDMC.dmc_set_homemode((ushort)CardNo, (ushort)axis, (ushort)home_dir, vel, (ushort)mode, (ushort)EZ_count);
        }
        ...
        {
            return 0; //dmc_get_homemode((ushort)CardNo, (ushort)axis, ref home_dir, ref vel, ref mode, ref EZ_count);
        }
        ...
        {
            int addr1 = -1;
            int.TryParse(axisIOStatusBuffer.data[axis, 14], out addr1);
            if (axisIOStatusBuffer.coilAddressSubOne)
            {
                addr1 -= 1;
            }
            ModubsComm.Instance.set_do_bit(CardNo, addr1, 0);   //dmc_home_move
            return 0;
        }         
        ...
        {
            return 0; //dmc_set_homelatch_mode((ushort)CardNo, (ushort)axis, enable, logic, source);
        }
        ...
        {
            return 0; //dmc_get_homelatch_mode((ushort)CardNo, (ushort)axis, ref enable, ref logic, ref source);
        }
        ...
        {
            return 0; //dmc_reset_homelatch_flag((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //dmc_get_homelatch_flag((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //dmc_get_homelatch_value((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //dmc_set_el_mode((ushort)CardNo, (ushort)axis, el_enable, el_logic, el_mode);
        }
        ...
        {
            return 0; //dmc_get_el_mode((ushort)CardNo, (ushort)axis, ref el_enable, ref el_logic, ref el_mode);
        }
        ...
        {
            return 0; //dmc_set_softlimit((ushort)CardNo, (ushort)axis, enable, source_sel, SL_action, N_limit, P_limit);
        }
        ...
        {
            return 0; //dmc_get_softlimit((ushort)CardNo, (ushort)axis, ref enable, ref source_sel, ref SL_action, ref N_limit, ref P_limit);
        }
        ...
        {
            //axisStatusParameter[axis][(int)AxisStatusParameterEnum.目标位置] = 0;
            //axisStatusParameter[axis][(int)AxisStatusParameterEnum.原点位] = 1;
            //axisStatusParameterStr[axis] = "000000000000000000010000000" + axisStatusParameter[axis][(int)AxisStatusParameterEnum.原点位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.急停].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.负限位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.正限位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.报警].ToString();
            return 0; //dmc_set_position((ushort)CardNO, (ushort)axis, current_position);
        }
        ...
        {
            int addr1 = -1;
            int.TryParse(axisIOStatusBuffer.data[axis, 24], out addr1);
            if (ModubsComm.Instance.get_do_regFloatData(addr1, 1).Count > 0)
            {
            ...
            //return axisStatusParameter[axis][(int)AxisStatusParameterEnum.目标位置]; //dmc_get_position((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //dmc_read_current_speed((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //dmc_LinkState((ushort)CardNo, ref State);
        }
        ...
        {
            int addr1 = -1;
            int.TryParse(axisIOStatusBuffer.data[axis, 23], out addr1);
            //23是：ptp运动到达位置
            ...
            //    return 0;
            //}
            //return 1; //dmc_check_done((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //dmc_check_done_multicoor((ushort)CardNo, (ushort)crd);
        }
        ...
        {
            if (ModubsComm.Instance.readOnlyCols.Count < 1) return 0;
            StringBuilder sb1 = new StringBuilder();
            for(int i=31;i>=0;i--)
            {
                var idx = axisStatusDataModbusCoilIdx[axis, i];
                //如果映射数组的值是负数，则表示此位为0，否则的话此位就是设置了正常的输入IO的地址，就应该由modbus缓存中取得值。
                var data2 = 0;
                if (idx >= 0) data2 = ModubsComm.Instance.readOnlyCols[idx]==true?1:0;
                switch (i)
                {
                    case 31: //报警
                    case 30: //正限位
            ...
            return Convert.ToInt32(sb1.ToString(), 2);  //dmc_axis_io_status
        }

      
        //private int getAxisStateInfo(int axisno, AxisStatusInfoEnum item)
        //{
        ...
        {
            int addr1 = -1, addr2 = -1;
            int.TryParse(axisIOStatusBuffer.data[axis, 21], out addr1);
            int.TryParse(axisIOStatusBuffer.data[axis, 15], out addr2);
            if (axisIOStatusBuffer.coilAddressSubOne)
            {
                addr1 -= 1;addr2 -= 1;
            }

            ModubsComm.Instance.set_do_bit(CardNo, addr1, 1);  //ptp停止
            Thread.Sleep(50);
            ModubsComm.Instance.set_do_bit(0, addr2, 1);   //jog停止
            return 0;
            //control = false;
            //return 0;//(int)LTDMC.dmc_stop((ushort)CardNo, (ushort)axis, (ushort)stop_mode);
        }
        ...
        {
            return 0; // (int)LTDMC.dmc_stop_multicoor((ushort)CardNo, (ushort)crd, (ushort)stop_mode);
        }
        ...
        public int CardAxisEmgStop(int CardNo)
        {
            for(int i=0;i<3;i++)
            {
                CardAxisStop(CardNo, i, 0);
            }
            return 0;
            //return (int)LTDMC.dmc_emg_stop((ushort)CardNo);
        }
        ...
        /// 速度参数
        /// </summary>
        public enum axisParameterEnum
        {
            MinVel, MaxVel, Tacc, Tdec, StopVel
        }

        private static Dictionary<int, List<double>> axisParameterBuffer = new Dictionary<int, List<double>>();


        private bool UpdateAxisParameter(int axisNo, axisParameterEnum para, double data)
        {
            if (!axisParameterBuffer.ContainsKey(axisNo)) return false;
            axisParameterBuffer[axisNo][(int)para] = data;
            return true;
        }
        ...
        {
            int addr1 = -1, addr2 = -1, addr3 = -1;
            int.TryParse(axisIOStatusBuffer.data[axis, 16], out addr1);
            int.TryParse(axisIOStatusBuffer.data[axis, 17], out addr2);
            int.TryParse(axisIOStatusBuffer.data[axis, 18], out addr3);

            ModubsComm.Instance.set_do_regdata(addr1, (float)Min_Vel);
            //Thread.Sleep(5);
            ModubsComm.Instance.set_do_regdata(addr2, (float)Max_Vel);
            //Thread.Sleep(5);
            ModubsComm.Instance.set_do_regdata(addr3, (float)Tacc);
            return 0;

            //var f1 = UpdateAxisParameter(axis, axisParameterEnum.MinVel, Min_Vel);
            //f1 &= UpdateAxisParameter(axis, axisParameterEnum.MaxVel, Max_Vel);
            //f1 &= UpdateAxisParameter(axis, axisParameterEnum.Tacc, Tacc);
            //f1 &= UpdateAxisParameter(axis, axisParameterEnum.Tdec, Tdec);
            //f1 &= UpdateAxisParameter(axis, axisParameterEnum.StopVel, stop_vel);
            //if (!f1) return 1;
            //return 0; //dmc_set_profile((ushort)CardNo, (ushort)axis, Min_Vel, Max_Vel, Tacc, Tdec, stop_vel);
        }
        ...
        public int GetCardAxisProfile(int CardNo, int axis, ref double Min_Vel, ref double Max_Vel, ref double Tacc, ref double Tdec, ref double stop_vel)
        {
            ...
            Min_Vel =ModubsComm.Instance.get_do_regdata(addr1);
            Max_Vel= ModubsComm.Instance.get_do_regdata(addr2);
            Tacc = Tdec= ModubsComm.Instance.get_do_regdata(addr3);
            stop_vel = 0;
            return 0; //dmc_get_profile((ushort)CardNo, (ushort)axis, ref Min_Vel, ref Max_Vel, ref Tacc, ref Tdec, ref stop_vel);
        }
        ...
        {
            return 0; //dmc_set_s_profile((ushort)CardNo, (ushort)axis, (ushort)s_mode, s_para);
        }
        ...
        {
            return 0; //dmc_get_s_profile((ushort)CardNo, (ushort)axis, (ushort)s_mode, ref s_para);
        }
        #endregion

        #region 设置限位原点

        Dictionary<int, List<int>> axisSpacing = new Dictionary<int, List<int>>();
        private enum axisSpacingEnum
        {
            原点 = 0, 正限位, 负限位
        }
        ...
        /// <returns></returns>
        public int CardAxisSpacing(int axis, int ORGPlusPos, int PELAlarmPlusPos, int NELAlarmPlusPos)
        {
            if (axisSpacing.ContainsKey(axis))
            {
                axisSpacing[axis][(int)axisSpacingEnum.原点] = ORGPlusPos;
                axisSpacing[axis][(int)axisSpacingEnum.正限位] = PELAlarmPlusPos;
                axisSpacing[axis][(int)axisSpacingEnum.负限位] = NELAlarmPlusPos;
            }
            return 0;
        }
        #endregion
        ...
        public int CardAxisPMove(int CardNo, int axis, int Dist, int posi_mode)
        {
            ...
            int addr1 = -1,addr2=-1,addr3=-1;
            int.TryParse(axisIOStatusBuffer.data[axis, 15], out addr1);
            int.TryParse(axisIOStatusBuffer.data[axis, 19], out addr2);
            int.TryParse(axisIOStatusBuffer.data[axis, 21], out addr3);
            if (axisIOStatusBuffer.coilAddressSubOne)
            {
                addr1 -= 1;
                addr3 -= 1;
            ...
            ModubsComm.Instance.set_do_bit(0, addr1, 1);  //如果axis stop(jog运动)了，则要取消

            ModubsComm.Instance.set_do_regdata(addr2, Dist);

            ModubsComm.Instance.set_do_bit(0, addr3, 1);
            Thread.Sleep(30);
            ModubsComm.Instance.set_do_bit(0, addr3, 0);  //x轴ptp开始运动
                                         
            //while (true)
            //{
            //    Thread.Sleep(5);
            //    if (ModubsComm.Instance.readwriteCols[axisIOStatusBuffer.data[axis, 21]] && 
            //        !ModubsComm.Instance.readOnlyCols[axisIOStatusBuffer.data[axis, 22]] && 
            //        ModubsComm.Instance.readOnlyCols[axisIOStatusBuffer.data[axis, 23]])
            //    {
            //        //TxtMsg($"{DateTime.Now.ToString()}: [{ptname}] x轴ptp运动状态=false,x轴ptp已运动到目的地=true");
            //        break;
            //    }
            //    //Application.DoEvents();
            //}
            //ModubsComm.Instance.set_do_bit(0, axisIOStatusBuffer.data[axis, 15], 1);
            return 0;
            //axisStatusParameter[axis][(int)AxisStatusParameterEnum.目标位置] = Dist;
            //if (axisSpacing[axis][(int)axisSpacingEnum.原点] == axisStatusParameter[axis][(int)AxisStatusParameterEnum.目标位置])
            //{
            //    axisStatusParameter[axis][(int)AxisStatusParameterEnum.原点位] = 1;
            //    axisStatusParameterStr[axis] = "000000000000000000010000000" + axisStatusParameter[axis][(int)AxisStatusParameterEnum.原点位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.急停].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.负限位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.正限位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.报警].ToString();
            //}
            //else
            //{
            //    axisStatusParameter[axis][(int)AxisStatusParameterEnum.原点位] = 0;
            //    axisStatusParameterStr[axis] = "000000000000000000010000000" + axisStatusParameter[axis][(int)AxisStatusParameterEnum.原点位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.急停].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.负限位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.正限位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.报警].ToString();
            //}
            //if (axisSpacing[axis][(int)axisSpacingEnum.正限位] < axisStatusParameter[axis][(int)AxisStatusParameterEnum.目标位置])
            //{
            //    axisStatusParameter[axis][(int)AxisStatusParameterEnum.目标位置] = axisSpacing[axis][(int)axisSpacingEnum.正限位];
            //    axisStatusParameter[axis][(int)AxisStatusParameterEnum.正限位] = 1;
            //    axisStatusParameterStr[axis] = "000000000000000000010000000" + axisStatusParameter[axis][(int)AxisStatusParameterEnum.原点位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.急停].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.负限位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.正限位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.报警].ToString();

            //}
            //if (axisSpacing[axis][(int)axisSpacingEnum.负限位] > axisStatusParameter[axis][(int)AxisStatusParameterEnum.目标位置])
            //{
            //    axisStatusParameter[axis][(int)AxisStatusParameterEnum.目标位置] = axisSpacing[axis][(int)axisSpacingEnum.负限位];
            //    axisStatusParameter[axis][(int)AxisStatusParameterEnum.负限位] = 1;
            //    axisStatusParameterStr[axis] = "000000000000000000010000000" + axisStatusParameter[axis][(int)AxisStatusParameterEnum.原点位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.急停].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.负限位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.正限位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.报警].ToString();

            //}
            //return 0; //dmc_pmove((ushort)CardNo, (ushort)axis, Dist, (ushort)posi_mode);
        }

        private Dictionary<int, List<int>> axisStatusParameter = new Dictionary<int, List<int>>();
        private bool control = true;
        private bool[] isRun = new bool[64];
        string[] axisStatusParameterStr = new string[64];
        /// <summary>
        /// 从modbus的继电器区映射轴的io数据。此数组行是64个轴，列是32位二进制数，表示轴相关的状态数据
        /// 数组的值保存modbustcp只读继电器区的索引号。会由一个磁盘文件初始化数组的值
        /// </summary>
        int[,] axisStatusDataModbusCoilIdx = new int[64, 32];

        private void changePulse(int axis, int dir)
        {
            var speedDifference = axisParameterBuffer[axis][1] - axisParameterBuffer[axis][0];
            double time = 0;
            while (control)
            {
                if (axisSpacing[axis][(int)axisSpacingEnum.原点] == axisStatusParameter[axis][(int)AxisStatusParameterEnum.目标位置])
                ...
                if (speedDifference == 0)
                {
                    ...
                    if (dir == 0)
                    {
                        axisStatusParameter[axis][(int)AxisStatusParameterEnum.目标位置] -= (int)(axisParameterBuffer[axis][1] / 1000);
                    }
                    if (dir == 1)
                    {
                        axisStatusParameter[axis][(int)AxisStatusParameterEnum.目标位置] += (int)(axisParameterBuffer[axis][1] / 1000);
                ...
                else
                {
                    Thread.Sleep(1);
                    time += 0.001;
                    if (axisParameterBuffer[axis][2] >= time)
                    {
                        if (dir == 0)
                        {
                            axisStatusParameter[axis][(int)AxisStatusParameterEnum.目标位置] -= (int)(axisParameterBuffer[axis][0] / 1000);
                        }
                        if (dir == 1)
                        {
                            axisStatusParameter[axis][(int)AxisStatusParameterEnum.目标位置] += (int)(axisParameterBuffer[axis][0] / 1000);
        ...
        public int CardAxisVMove(int CardNo, int axis, int dir)
        {
            ...
            int addr4 = -1;
            int.TryParse(axisIOStatusBuffer.data[axis, 14], out addr1);
            int.TryParse(axisIOStatusBuffer.data[axis, 12], out addr2);
            int.TryParse(axisIOStatusBuffer.data[axis, 13], out addr3);
            int.TryParse(axisIOStatusBuffer.data[axis, 34], out addr4);
            if (axisIOStatusBuffer.coilAddressSubOne)
            {
                //如果开关有效，则把用户定义继电器地址减1，而寄存器地址不用减1
                //这是因为如果用户直接录入的是unity中定义的地址，则需要减1
                addr1 -= 1;
                addr2 -= 1;
                addr3 -= 1;
                addr4 -= 1;
            }
            ModubsComm.Instance.set_do_bit(CardNo, addr4, 0);
            Thread.Sleep(50);
            ModubsComm.Instance.set_do_bit(0, addr1, 1);   //关闭回原
            if (dir == 1)
            {
                ModubsComm.Instance.set_do_bit(0, addr2, 0);  //正方向
            }
            else
            {
                ModubsComm.Instance.set_do_bit(0, addr3, 0);  //负方向
            }
            return 0;
            //if (axisParameterBuffer.ContainsKey(axis))
            //{
            //    t = new Task(() => changePulse(axis, dir));
            //    control = true;
            //    t.Start();
            //}
            //return 0; //dmc_vmove((ushort)CardNo, (ushort)axis, (ushort)dir);
        }
        ...
        public int SetStatusParameter(int axis, bool f, AxisStatusParameterEnum kind)
        {
            if (f)
            {
                axisStatusParameter[axis][(int)kind] = 1;
                axisStatusParameterStr[axis] = "000000000000000000010000000" + axisStatusParameter[axis][(int)AxisStatusParameterEnum.原点位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.急停].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.负限位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.正限位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.报警].ToString();
                // axisStatusParameterStr[axis] = axisStatusParameter[axis][(int)AxisStatusEnum.报警].ToString() + axisStatusParameter[axis][(int)AxisStatusEnum.正限位].ToString() + axisStatusParameter[axis][(int)AxisStatusEnum.负限位].ToString() + axisStatusParameter[axis][(int)AxisStatusEnum.急停].ToString() + axisStatusParameter[axis][(int)AxisStatusEnum.原点位].ToString() + 0 + 0 + 0 + 0 + 0;
            }
            else
            {
                axisStatusParameter[axis][(int)kind] = 0;
                axisStatusParameterStr[axis] = "000000000000000000010000000" + axisStatusParameter[axis][(int)AxisStatusParameterEnum.原点位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.急停].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.负限位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.正限位].ToString() + axisStatusParameter[axis][(int)AxisStatusParameterEnum.报警].ToString();
                //axisStatusParameterStr[axis] = axisStatusParameter[axis][(int)AxisStatusEnum.报警].ToString() + axisStatusParameter[axis][(int)AxisStatusEnum.正限位].ToString() + axisStatusParameter[axis][(int)AxisStatusEnum.负限位].ToString() + axisStatusParameter[axis][(int)AxisStatusEnum.急停].ToString() + axisStatusParameter[axis][(int)AxisStatusEnum.原点位].ToString() + 0 + 0 + 0 + 0 + 0;
            }
            return 0;
        ...
        {
            return 0; //dmc_change_speed((ushort)CardNo, (ushort)axis, Curr_Vel, Taccdec);
        }
        ...
        {
            return 0; //dmc_reset_target_position((ushort)CardNo, (ushort)axis, dist, (ushort)posi_mode);
        }
        ...
        {
            return 0; //dmc_update_target_position((ushort)CardNo, (ushort)axis, dist, (ushort)posi_mode);
        }
        #endregion
        ...
        {
            return 0; //dmc_set_vector_profile_multicoor((ushort)CardNo, (ushort)Crd, Min_Vel, Max_Vel, Tacc, Tdec, Stop_Vel);
        }
        ...
        {
            return 0; //dmc_get_vector_profile_multicoor((ushort)CardNo, (ushort)Crd, ref Min_Vel, ref Max_Vel, ref Tacc, ref Tdec, ref Stop_Vel);
        }
        ...
        {
            return 0; //dmc_line_multicoor((ushort)CardNo, (ushort)Crd, (ushort)axisNum, axisList, DistList, (ushort)posi_mode);
        }
        ...
        {
            return 0; //dmc_arc_move_multicoor((ushort)CardNo, (ushort)Crd, AxisList, Target_Pos, Cen_Pos, (ushort)Arc_Dir, (ushort)posi_mode);
        }
        ...
        {
            return 0; //dmc_PttTable((ushort)CardNo, (ushort)axis, count, pTime, pPos);
        }
        ...
        {
            return 0; //dmc_PtsTable((ushort)CardNo, (ushort)axis, count, pTime, pPos, pPercent);
        }
        ...
        {
            return 0; //dmc_PvtTable((ushort)CardNo, (ushort)axis, count, pTime, pPos, pVel);
        }
        ...
        {
            return 0; //dmc_PvtsTable((ushort)CardNo, (ushort)axis, count, pTime, pPos, velBegin, velEnd);
        }
        ...
        {
            return 0; //dmc_PvtMove((ushort)CardNo, (ushort)axisnum, AxisList);
        }
        ...
        public int CardAxisWriteSevonPin(int CardNo, int axis, int on_off)
        {
            ...
            return 0; //dmc_write_sevon_pin((ushort)CardNo, (ushort)axis, (ushort)on_off);
        }
        ...
        {
            return 0; //dmc_read_sevon_pin((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //dmc_read_rdy_pin((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //dmc_set_inp_mode((ushort)CardNo, (ushort)axis, (ushort)enable, (ushort)inp_logic);
        }
        ...
        {
            return 0; //dmc_get_inp_mode((ushort)CardNo, (ushort)axis, ref enable, ref inp_logic);
        }
        ...
        {
            return 0; //dmc_set_alm_mode((ushort)CardNo, (ushort)axis, (ushort)enable, (ushort)alm_logic, (ushort)alm_action);
        }
        ...
        {
            return 0; //dmc_get_alm_mode((ushort)CardNo, (ushort)axis, ref enable, ref alm_logic, ref alm_action);
        }
        ...
        {
            return 0; //dmc_write_erc_pin((ushort)CardNo, (ushort)axis, (ushort)sel);
        }
        ...
        {
            return 0; //dmc_read_erc_pin((ushort)CardNo, (ushort)axis);
        }
        ...
        public int GetCardInBit(int CardNo, int bitno)
        {
            ...
            //if (!MainBordInputIOBitBuffer.ContainsKey((ushort)bitno)) return -1;
            //return MainBordInputIOBitBuffer[(ushort)bitno].BitValue ? 1 : 0;
            //return 0; //dmc_read_inbit((ushort)CardNo, (ushort)bitno);
        }
        ...
        {
            //if (!MainBordInputIOBitBuffer.ContainsKey((ushort)bitno)) return -1;
            //var tmpdata = MainBordInputIOBitBuffer[(ushort)bitno];
            //MainBordInputIOBitBuffer[(ushort)bitno] = new VitualCardInputIOStruct()
            //{
            //    InputPortNo = tmpdata.InputPortNo,
            //    Name = tmpdata.Name,
            //    BitValue = on_off == 1 ? true : false
            //};
            return 0;
        }
        ...
        {
            //if (!ExtendInputIOBitBuffer.ContainsKey($"{nodeno},{bitno}")) return -1;
            //var tmpdata = ExtendInputIOBitBuffer[$"{nodeno},{bitno}"];
            //ExtendInputIOBitBuffer[$"{nodeno},{bitno}"] = new VitualCardExtendInputIOStruct()
            //{
            //    NodeNo = nodeno,
            //    InputPortNo = tmpdata.InputPortNo,
            //    Name = tmpdata.Name,
        ...
        {
            ModubsComm.Instance.set_do_bit(CardNo, bitno, on_off);
            return 0;
            //if (!MainBordOutputIOBitBuffer.ContainsKey((ushort)bitno)) return -1;
            //var tmpdata = MainBordOutputIOBitBuffer[(ushort)bitno];
            //MainBordOutputIOBitBuffer[(ushort)bitno] = new VitualCardOutputIOStruct()
            //{
            //    OutputPortNo = tmpdata.OutputPortNo,
            //    Name = tmpdata.Name,
            //    BitValue = on_off == 1 ? true : false
            //};
            //return 0;
            //return 0; //dmc_write_outbit((ushort)CardNo, (ushort)bitno, (ushort)on_off);
        }
        ...
        {
           return  ModubsComm.Instance.get_do_bit(CardNo, bitno);
            //if (!MainBordOutputIOBitBuffer.ContainsKey((ushort)bitno)) return -1;
            //return MainBordOutputIOBitBuffer[(ushort)bitno].BitValue ? 1 : 0;
            //return 0; //dmc_read_outbit((ushort)CardNo, (ushort)bitno);
        }
        ...
        public uint GetCardInPort(int CardNo, int portno)
        {
            ...
            if (portno == 0)
            {
                ...
                //{
                //    bary.Add(MainBordInputIOBitBuffer[i].BitValue);
                //}
                //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 0) == 0 ? true:false);
                //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 1) == 0 ? true : false);
                //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 2) == 0 ? true : false);
                //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 3) == 0 ? true : false);
                //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 4) == 0 ? true : false);
                //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 5) == 0 ? true : false);
                //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 6) == 0 ? true : false);
                //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 7) == 0 ? true : false);
                //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 8) == 0 ? true : false);
                //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 9) == 0 ? true : false);
                //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 10) == 0 ? true : false);
                //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 11) == 0 ? true : false);
                //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 12) == 0 ? true : false);
                //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 13) == 0 ? true : false);
                //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 14) == 0 ? true : false);
                //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 15) == 0 ? true : false);
                bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 0) == 1 ? true : false);
                bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 1) == 1 ? true : false);
                bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 2) == 1 ? true : false);
                bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 3) == 1 ? true : false);
                bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 4) == 1 ? true : false);
                bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 5) == 1 ? true : false);
                bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 6) == 1 ? true : false);
                bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 7) == 1 ? true : false);
                bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 8) == 1 ? true : false);
                bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 9) == 1 ? true : false);
                bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 10) == 1 ? true : false);
                bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 11) == 1 ? true : false);
                bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 12) == 1 ? true : false);
                bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 13) == 1 ? true : false);
                bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 14) == 1 ? true : false);
                bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 15) ==1 ? true : false);
                bary.Add(false);
                bary.Add(false);
                bary.Add(false);
                bary.Add(false);
            ...
            {
                for (ushort i = 32; i < 64; i++)
                {
                    bary.Add(false);
                }
            }

            BitArray ary = new BitArray(bary.ToArray());

            return (uint)BitToIntOne(ary);
            //return 0; //dmc_read_inport((ushort)CardNo, (ushort)portno);
        }

        public int BitToIntOne(BitArray bit)
        {
            int res = 0;
        ...
        public uint GetCardOutPort(int CardNo, int portno)
        {
            ...
            //for (ushort i = 0; i <= 31; i++)
            //{
            //    bary.Add(MainBordOutputIOBitBuffer[i].BitValue);
            //}
            //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 0) == 0 ? true : false);
            //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 1) == 0 ? true : false);
            //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 2) == 0 ? true : false);
            //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 3) == 0 ? true : false);
            //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 4) == 0 ? true : false);
            //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 5) == 0 ? true : false);
            //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 6) == 0 ? true : false);
            //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 7) == 0 ? true : false);
            //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 8) == 0 ? true : false);
            //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 9) == 0 ? true : false);
            //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 10) == 0 ? true : false);
            //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 11) == 0 ? true : false);
            //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 12) == 0 ? true : false);
            //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 13) == 0 ? true : false);
            //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 14) == 0 ? true : false);
            //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 15) == 0 ? true : false);

            //注意unity中的电平0,1跟SMotorse的需求是反的
            bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 0) == 1 ? true : false);
            bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 1) == 1 ? true : false);
            bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 2) == 1 ? true : false);
            bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 3) == 1 ? true : false);
            bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 4) == 1 ? true : false);
            bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 5) == 1 ? true : false);
            bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 6) == 1 ? true : false);
            bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 7) == 1 ? true : false);
            bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 8) == 1 ? true : false);
            bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 9) == 1 ? true : false);
            bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 10) == 1 ? true : false);
            bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 11) == 1 ? true : false);
            bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 12) == 1 ? true : false);
            bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 13) == 1 ? true : false);
            bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 14) == 1 ? true : false);
            bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 15) ==1 ? true : false);
            ...
            return (uint)BitToIntOne(ary);
            //return 0; //dmc_read_outport((ushort)CardNo, (ushort)portno);
        }
        ...
        {
            var str = Convert.ToString(outport_val, 2).PadLeft(32, '0');
            List<bool> bary = new List<bool>();
            
            for (int i = 31; i >=16; i--)
            {
                bary.Add(str[i] == '0' ? true : false);
            }
            ModubsComm.Instance.set_do_mulBit(CardNo, 0, bary);
            return 0; //dmc_write_outport((ushort)CardNo, (ushort)portno, outport_val);
        }
        ...
        {
            return 0; //dmc_reverse_outbit((ushort)CardNo, (ushort)bitno, reverse_time);
        }
        ...
        {
            return 0; //dmc_set_io_count_mode((ushort)CardNo, (ushort)bitno, (ushort)mode, filter_time);
        }
        ...
        public int GetCardIoCountMode(int CardNo, int bitno, ref UInt16 mode, ref double filter_time)
        {
            ...
            return 0; //dmc_get_io_count_mode((ushort)CardNo, (ushort)bitno, ref mode, ref filter_time);
        }
        /// <summary>
        /// 设置 IO 计数值
        /// </summary>
        ...
        {
            return 0; //dmc_set_io_count_value((ushort)CardNo, (ushort)bitno, (uint)CountValue);
        }
        ...
        {
            return 0; //dmc_get_io_count_value((ushort)CardNo, (ushort)bitno, ref CountValue);
        }
        ...
        {
            return 0; //dmc_set_handwheel_inmode((ushort)CardNo, (ushort)axis, (ushort)inmode, multi, vh);
        }
        ...
        {
            return 0; //dmc_get_handwheel_inmode((ushort)CardNo, (ushort)axis, ref inmode, ref multi, ref vh);
        }
        ...
        {
            return 0; //dmc_handwheel_move((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //dmc_set_handwheel_channel((ushort)CardNo, (ushort)index);
        }
        ...
        {
            return 0; //dmc_get_handwheel_channel((ushort)CardNo, ref index);
        }
        ...
        {
            return 0; //dmc_set_handwheel_inmode_extern((ushort)CardNo, inmode, AxisNum, AxisList, multi);
        }
        ...
        {
            return 0; //dmc_get_handwheel_inmode_extern((ushort)CardNo, ref inmode, ref AxisNum, AxisList, multi);
        }
        ...
        {
            return 0; //dmc_set_counter_inmode((ushort)CardNo, (ushort)axis, (ushort)mode);
        }
        ...
        public int GetCardAxisCounterInMode(int CardNo, int axis, ref UInt16 mode)
        {
            ...
            return 0; //dmc_get_counter_inmode((ushort)CardNo, (ushort)axis, ref mode);
        }
        ...
        {
            //仿真器不支持编码器操作，因此这里等于对SetCardAxisPosition的操作
            SetCardAxisPosition(CardNo, axis, encoder_value);
            return 0;
            //axisStatusParameter[axis][(int)AxisStatusParameterEnum.编码器位置] = 0;
            //return 0; //dmc_set_encoder((ushort)CardNo, (ushort)axis, encoder_value);
        }
        /// <summary>
        /// 获取指定轴编码器脉冲计数值
        /// <para>说 明：</para>
        ...
        public int GetCardAxisEncoder(int CardNo, int axis)
        {
            //返回ptp的当前益
            int addr = -1;
            int.TryParse(axisIOStatusBuffer.data[axis, 24], out addr);
            int data=(int)ModubsComm.Instance.get_do_regdata(addr);
            return 0; //dmc_get_encoder((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //dmc_set_ez_mode((ushort)CardNo, (ushort)axis, (ushort)ez_logic, (ushort)ez_mode, filter);
        }
        ...
        {
            return 0; //dmc_get_ez_mode((ushort)CardNo, (ushort)axis, ref ez_logic, ref ez_mode, ref filter);
        }
        #endregion
        ...
        {
            return 0; //dmc_set_ltc_mode((ushort)CardNo, (ushort)axis, ltc_logic, ltc_mode, filter);
        }
        public int SetCardAxisLatchMode(int CardNo, int axis, int all_enable, int latch_source, int triger_chunnel)
        {
            return 0; //dmc_set_latch_mode((ushort)CardNo, (ushort)axis, (ushort)all_enable, (ushort)latch_source, (ushort)triger_chunnel);
        }
        /// <summary>
        /// 获取指定轴的 LTC 信号设置
        /// </summary>
        ...
        {
            return 0; //dmc_get_ltc_mode((ushort)CardNo, (ushort)axis, ref ltc_logic, ref ltc_mode, ref filter);
        }
        ...
        {
            return 0; //dmc_set_latch_mode((ushort)CardNo, (ushort)axis, all_enable, latch_source, triger_chunnel);
        }
        ...
        {
            return 0; //dmc_get_latch_mode((ushort)CardNo, (ushort)axis, ref all_enable, ref latch_source, ref triger_chunnel);
        }
        ...
        {
            return 0; //dmc_set_latch_stop_time((ushort)CardNo, (ushort)axis, time);
        }
        ...
        {
            return 0; //dmc_get_latch_stop_time((ushort)CardNo, (ushort)axis, ref time);
        }
        ...
        {
            return 0; //dmc_SetLtcOutMode((ushort)CardNo, (ushort)axis, (ushort)enable, (ushort)bitno);
        }
        ...
        /// <returns>错误代码</returns>
        public int GetCardAxisOutMode(int CardNo, int axis, UInt16 enable, UInt16 bitno)
        {
            return 0; //dmc_GetLtcOutMode((ushort)CardNo, (ushort)axis, ref enable, ref bitno);
        }
        ...
        {
            return 0; //dmc_get_latch_value((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //dmc_get_latch_flag((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //dmc_reset_latch_flag((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //dmc_get_latch_flag_extern((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //dmc_get_latch_value_extern((ushort)CardNo, (ushort)axis, (ushort)Index);
        }
        ...
        {
            //dmc_hcmp_2d_set_enable
            return 0; //dmc_hcmp_2d_set_enable((ushort)CardNo, (ushort)hcmp,(ushort)cmpEnable);
        }
        ...
        {
            return 0; //dmc_compare_set_config((ushort)CardNo, (ushort)axis, (ushort)enable, (ushort)cmp_source);
        }
        ...
        {
            return 0; //dmc_compare_get_config((ushort)CardNo, (ushort)axis, ref enable, ref cmp_source);
        }
        ...
        {
            return 0; //dmc_compare_clear_points((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //dmc_compare_add_point((ushort)CardNo, (ushort)axis, pos, (ushort)dir, action, actpara);
        }
        ...
        {
            return 0; //dmc_compare_get_current_point((ushort)CardNo, (ushort)axis, ref pos);
        }
        ...
        {
            return 0; //dmc_compare_get_points_runned((ushort)CardNo, (ushort)axis, ref pointNum);
        }
        ...
        {
            return 0; //dmc_compare_get_points_remained((ushort)CardNo, (ushort)axis, ref pointNum);
        }
        ...
        {
            return 0; //dmc_compare_set_config_extern((ushort)CardNo, (ushort)enable, (ushort)cmp_source);
        }
        ...
        {
            return 0; //dmc_compare_get_config_extern((ushort)CardNo, ref enable, ref cmp_source);
        }
        ...
        {
            return 0; //dmc_compare_clear_points_extern((ushort)CardNo);
        }
        ...
        {
            return 0; //dmc_compare_add_point_extern((ushort)CardNo, axis, pos, dir, action, actpara);
        }
        ...
        {
            return 0; //dmc_compare_get_current_point_extern((ushort)CardNo, pos);
        }
        ...
        {
            return 0; //dmc_compare_get_points_runned_extern((ushort)CardNo, ref pointNum);
        }
        ...
        {
            return 0; //dmc_compare_get_points_remained_extern((ushort)CardNo, ref pointNum);
        }
        ...
        {
            return 0; //dmc_hcmp_set_mode((ushort)CardNo, (ushort)hcmp, (ushort)cmp_enable);
        }
        /// <summary>
        /// 清除已添加的所有高速位置比较点
        /// </summary>
        ...
        {
            return 0; //dmc_hcmp_clear_points((ushort)CardNo, (ushort)hcmp);
        }
        /// <summary>
        ///读取高速比较模式设置
        /// </summary>
        ...
        {
            return 0; //dmc_hcmp_get_mode((ushort)CardNo, (ushort)hcmp, ref cmp_enable);
        }
        ...
        {
            return 0; //dmc_hcmp_set_config((ushort)CardNo, (ushort)hcmp, (ushort)axis, (ushort)cmp_source, (ushort)cmp_logic, time);
        }
        ...
        {
            return 0; //dmc_hcmp_get_config((ushort)CardNo, (ushort)hcmp, ref axis, ref cmp_source, ref cmp_logic, ref time);
        }
        ...
        {
            return 0; //dmc_hcmp_add_point((ushort)CardNo, (ushort)hcmp, cmp_pos);
        }
        ...
        {
            return 0; //dmc_hcmp_set_liner((ushort)CardNo, (ushort)hcmp, Increment, Count);
        }
        ...
        {
            return 0; //dmc_hcmp_get_liner((ushort)CardNo, (ushort)hcmp, ref Increment, ref Count);
        }
        ...
        {
            return 0; //dmc_hcmp_get_current_state((ushort)CardNo, (ushort)hcmp, ref remained_points, ref current_point, ref runned_points);
        }
        ...
        {
            return 0; //dmc_write_cmp_pin((ushort)CardNo, (ushort)hcmp, (ushort)on_off);
        }
        ...
        {
            return 0; //dmc_read_cmp_pin((ushort)CardNo, (ushort)hcmp);
        }
        #endregion
        ...
        {
            return 0; //dmc_set_emg_mode((ushort)CardNo, (ushort)axis, (ushort)enable, (ushort)emg_logic);
        }
        ...
        {
            return 0; //dmc_get_emg_mode((ushort)CardNo, (ushort)axis, ref enable, ref emg_logic);
        }

        /// <summary>
        /// 设置减速停止信号
        /// <para>注 意：</para>
        ///  <para>减速停止信号（DSTP）的减速时间由函数 dmc_set_dec_stop_time 设置</para>
        /// </summary>
        /// <param name="CardNo">控制卡卡号</param>
        /// <param name="axis">DMC3C00/3800 卡：指定轴号，取值范围：0~7; DMC3600 卡：指定轴号，取值范围：0~5 ;DMC3400A:指定轴号，取值范围：0~3</param>
        ...
        {
            return 0; //dmc_set_io_dstp_mode((ushort)CardNo, (ushort)axis, (ushort)enable, (ushort)logic);
        }
        ...
        {
            return 0; //dmc_get_io_dstp_mode((ushort)CardNo, (ushort)axis, ref enable, ref logic);
        }

        /// <summary>
        /// 设置减速停止时间
        /// <para>注 意：</para>
        /// <para>当发生异常停止时，如：调用 dmc_stop 函数、限位信号（软硬件）被触发、减速停止信号(DSTP)被触发等进行减速停止时，减速停止时间都为 dmc_set_dec_stop_time函数里设置的减速时间</para>
        /// </summary>
        /// <param name="CardNo">控制卡卡号</param>
        /// <param name="axis">指定轴号，取值范围：DMC3C00/3800：0~7，DMC3600：0~5,DMC3400A： 0~3</param>
        ...
        {
            return 0; //dmc_set_dec_stop_time((ushort)CardNo, (ushort)axis, stop_time);
        }
        ...
        {
            return 0; //dmc_get_dec_stop_time((ushort)CardNo, (ushort)axis, ref stop_time);
        }
        ...
        {
            return 0; //dmc_set_axis_io_map((ushort)CardNo, (ushort)axis, (ushort)IoType, (ushort)MapIoType, (ushort)MapIoIndex, Filter);
        }
        ...
        {
            return 0; //dmc_get_axis_io_map((ushort)CardNo, (ushort)axis, (ushort)IoType, ref MapIoType, ref MapIoIndex, ref Filter);
        }
        ...
        {
            return 0; //dmc_set_special_input_filter((ushort)CardNo, Filter);
        }
        ...
        /// <para>2：dmc_set_io_map_virtual </para>
        ...
        {
            return 0; //dmc_set_io_map_virtual((ushort)CardNo, (ushort)bitno, (ushort)MapIoType, (ushort)MapIoIndex, Filter);
        }
        ...
        /// <para>2）dmc_read_inbit_virtual 与 dmc_read_inbit 函数的区别是：dmc_read_inbit 函数是不经过滤波直接读取硬件端口的电平状态，dmc_read_inbit_virtual 函数通过虚拟 IO 映射后读取相应端口滤波后的电平状态</para>
        /// </summary>
        /// <param name="CardNo">控制卡卡号</param>
        /// <param name="bitno">虚拟 IO 口号,取值范围：0~15</param>
        ...
        {
            return 0; //dmc_get_io_map_virtual((ushort)CardNo, (ushort)bitno, ref MapIoType, ref MapIoIndex, ref filter_time);
        }
        ...
        {
            return 0; //dmc_read_inbit_virtual((ushort)CardNo, (ushort)bitno);
        }
        #endregion
        ...
        {
            return 0; //dmc_set_factor_error((ushort)CardNo, (ushort)axis, factor, error);
        }
        ...
        {
            return 0; //dmc_get_factor_error((ushort)CardNo, (ushort)axis, ref factor, ref error);
        }
        ...
        {
            return 0; //dmc_check_success_pulse((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //dmc_check_success_encoder((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return 0; //nmc_set_connect_state((ushort)CardNo, (ushort)NodeNum, (ushort)state, (ushort)baud);
        }
        ...
        public int GetCard_CANIO_ConnectState(int CardNo, ref UInt16 NodeNum, ref UInt16 state)
        {
            NodeNum = 8;
            state = 1;
            return 0; //nmc_get_connect_state((ushort)CardNo, ref NodeNum, ref state);
        }
        /// <summary>
        ...
        {
            //仿真器的IO是： 主板16入16出，扩展IO是若干个32入32出
            ModubsComm.Instance.set_do_bit(CardNo, 16 + 32 * (NodeID - 1)+IoBit, IoValue);
            return 0;
            //if (!ExtendOutputIOBitBuffer.ContainsKey($"{NodeID},{IoBit}")) return -1;
            //var tmpdata = ExtendOutputIOBitBuffer[$"{NodeID},{IoBit}"];
            //ExtendOutputIOBitBuffer[$"{NodeID},{IoBit}"] = new VitualCardExtendOutputIOStruct()
            //{
            //    NodeNo = NodeID,
            //    OutputPortNo = tmpdata.OutputPortNo,
            //    Name = tmpdata.Name,
            //    BitValue = IoValue == 1 ? true : false
            //};
            //return 0;
            //return 0; //nmc_write_outbit((ushort)CardNo, (ushort)NodeID, (ushort)IoBit, (ushort)IoValue);
        }
        ...
        {
           var data= ModubsComm.Instance.get_do_bit(CardNo, 16 + 32 * (NodeID - 1) + IoBit);
            IoValue = (ushort)data;
            return data; //nmc_read_outbit((ushort)CardNo, (ushort)NodeID, (ushort)IoBit, ref IoValue);
        }
        ...
        {
            var data= ModubsComm.Instance.get_di_bit(CardNo, 16 + 32 * (NodeID - 1) + IoBit);
            IoValue = (ushort)data;
            return data; //nmc_read_inbit((ushort)CardNo, (ushort)NodeID, (ushort)IoBit, ref IoValue);
        }
        ...
        {
            return 0; //nmc_write_outport((ushort)CardNo, (ushort)NodeID, (ushort)PortNo, (uint)IoValue);
        }
        ...
        public int GetCard_CANIO_OutPort(int CardNo, int NodeID, int PortNo, ref UInt32 IoValue)
        {
            ...
            if (PortNo == 0)
            {
                ...
                for (ushort i = 0; i <= 31; i++)
                {
                    //bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 16 + 32 * (NodeID - 1) + i) == 0?true:false);

                    //unity那边的0,1需要反过来
                    bary.Add(ModubsComm.Instance.get_do_bit(CardNo, 16 + 32 * (NodeID - 1) + i) == 1 ? true : false);

                    //bary.Add(ExtendOutputIOBitBuffer[$"{NodeID},{i}"].BitValue);
                }
            }
            //else if (PortNo == 1)
            //{
            ...
            IoValue = (uint)BitToIntOne(ary);
            return 0;

            // return 0; //nmc_read_outport((ushort)CardNo, (ushort)NodeID, (ushort)PortNo, ref IoValue);
        }
        ...
        public int GetCard_CANIO_InPort(int CardNo, int NodeID, int PortNo, ref UInt32 IoValue)
        {
            ...
            if (PortNo == 0)
            {
                for (ushort i = 0; i <= 31; i++)
                {
                    //bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 16 + 32 * (NodeID - 1) + i) == 0 ? true : false);

                    bary.Add(ModubsComm.Instance.get_di_bit(CardNo, 16 + 32 * (NodeID - 1) + i) == 1 ? true : false);
                    //bary.Add(ExtendInputIOBitBuffer[$"{NodeID},{i}"].BitValue);
                }
            }
            //else if (PortNo == 1)
            //{
            ...
            BitArray ary = new BitArray(bary.ToArray());
            IoValue = (UInt32)BitToIntOne(ary);
            return 0;
            //return 0; //nmc_read_inport((ushort)CardNo, (ushort)NodeID, (ushort)PortNo, ref IoValue);
        }
        ...
        {
            return 0; //dmc_set_da_enable((ushort)CardNo, (ushort)enable);
        }
        /// <summary>
        /// 读取 DA 输出使能设置
        /// </summary>
        ...
        {
            return 0; //dmc_get_da_enable((ushort)CardNo, ref enable);
        }
        ...
        {
            return 0; //dmc_set_da_output((ushort)CardNo, (ushort)channel, Vout);
        }
        ...
        {
            return 0; //dmc_get_da_output((ushort)CardNo, (ushort)channel, ref Vout);
        }
        ...
        /// <returns>错误代码</returns>
        public int GetCardDAInPut(int CardNo, int channel, ref double Vout)
        {
            return 0; //dmc_get_ad_input((ushort)CardNo, (ushort)channel, ref Vout);
        }
        ...
        {
            return 0; //dmc_write_sn((ushort)CardNo, new_sn);
        }
        ...
        {
            return 0; //dmc_check_sn((ushort)CardNo, check_sn);
        }
        ...
        {
            return 0; //dmc_set_debug_mode((ushort)CardNo, FileName);
        }
        ...
        {
            return 0; //dmc_get_debug_mode(ref mode, FileName);
        }
        ...
        {
            return 0; //nmc_read_outbit((ushort)CardNo, (ushort)NodeID, (ushort)IoBit, ref IoValue);
        }
        ...
        {
            var str = Convert.ToString(IoValue, 2).PadLeft(32, '0');
            List<bool> bary = new List<bool>();

            //for (int i = 16+(NodeID-1)*32; i < 16 + (NodeID - 1) * 32+32; i++)
            for(int i=31;i>=0;i--)
            {
                bary.Add(str[i] == '0' ? true : false);
                //bary.Add(str[i] == '1' ? true : false);   //unity那边要倒过来1是低电平
            }
            ModubsComm.Instance.set_do_mulBit(CardNo, 16 + (NodeID - 1) * 32, bary);

            return 0; //nmc_write_outport((ushort)CardNo, (ushort)NodeID, (ushort)PortNo, (ushort)IoValue);
        }
        ...
        {
            return 0; //nmc_set_da_output((ushort)CardNo, (ushort)NodeID, (ushort)PortNo, IoValue);
        }
        ...
        {
            return 0; //nmc_get_da_output((ushort)CardNo, (ushort)NodeID, (ushort)PortNo, ref IoValue);
        }
        ...
        {
            return 0; //nmc_set_da_mode((ushort)CardNo, (ushort)NodeID, (ushort)PortNo, (ushort)mode, buffer_nums);
        }
        ...
        {
            return 0; //nmc_get_da_mode((ushort)CardNo, (ushort)NodeID, (ushort)PortNo, ref mode, buffer_nums);
        }
        ...
        {
            return 0; //nmc_read_inbit((ushort)CardNo, (ushort)NodeID, (ushort)IoBit, ref IoValue);
        }
        ...
        {
            return 0; //nmc_get_ad_input((ushort)CardNo, (ushort)NodeID, (ushort)PortNo, ref IoValue);
        }
        ...
        {
            return 0; //nmc_set_ad_mode((ushort)CardNo, (ushort)NodeID, (ushort)PortNo, (ushort)mode, buffer_nums);
        ...
        {
            return 0; //nmc_get_ad_mode((ushort)CardNo, (ushort)NodeID, (ushort)PortNo, ref mode, buffer_nums);
        ...
        {
            return 0; //nmc_write_to_flash((ushort)CardNo, (ushort)PortNum, (ushort)NodeNum);
        }
    ...
    public class mappingAxisIODataStruct
    {
        //public int[,] data { get; set; } = new int[64, 32];


        public string[,] data { get; set; } = new string[64, 35];
        public string[,] lineData { get; set; } = new string[16, 3];

        /// <summary>
        /// 继电器地址减1，寄存器地址不减1
        /// </summary>
        public bool coilAddressSubOne { get; set; } = true;
    }
}
// ◆◇※▣▤▥▦▧▨▩░▒▓✦✧⚝☢☣➤◈❖◆◇※▣▤▥▦▧▨▩░▒▓​⁣​
