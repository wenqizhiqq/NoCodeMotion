...
//   源: MotionCardRes/VirtualMotionCard/VirtualCardSDK.cs
...
namespace Samsun.Domain.MotionCard.Common.VirtualMotionCard
{
    /// <summary>
    /// 虚拟运动卡SDK
    /// </summary>
    public class VirtualMotionCardSDK
    {
        ...
        private static VirtualMotionCardSDK _instance;
        public static VirtualMotionCardSDK Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new VirtualMotionCardSDK();
                return VirtualMotionCardSDK._instance;
            }
            set { VirtualMotionCardSDK._instance = value; }
        }
        ...
        public int CardAxisHomeMove(int CardNo, int axis)
        {
            Task.Run(() =>
            {
                while (true)
                {
                    if (axisStatusParameter[axis][(int)AxisStatusParameterEnum.目标位置] > axisSpacing[axis][(int)axisSpacingEnum.原点])
                    {
                        Thread.Sleep(1);
            ...
            return 0; //dmc_home_move((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return Convert.ToInt32(axisStatusParameterStr[axis], 2);//(int)LTDMC.dmc_axis_io_status((ushort)CardNo, (ushort)axis);
        }
        ...
        {
            return (int)LTDMC.dmc_stop_multicoor((ushort)CardNo, (ushort)crd, (ushort)stop_mode);
        }
        ...
        private bool[] isRun = new bool[32];
        string[] axisStatusParameterStr = new string[32];
        ...
        // 各轴独立的伺服使能电平，键 = CardNo * 1000000 + axis（long，防溢出/防撞键）。
        // 原实现是用一个共享的 int Sevon 字段（不区分轴），写任一根轴都会把整张卡的
        // 使能状态一起改掉 —— 表现为「点一根轴的使能，所有轴一起使能/失能」。
        // 这里改为按 CardNo + axis 分开存储，使能/失能真正只作用于单轴。
        private readonly Dictionary<long, int> _sevonByAxis = new Dictionary<long, int>();

        // ===== 防呆：返回码（沿用本类「错误代码」约定：0 = 成功，非 0 = 失败）=====
        private const int OK = 0;
        private const int Err_InvalidCardNo = -1;   // 卡号非法（负数）
        private const int Err_InvalidAxis = -2;     // 轴号非法（负数）
        private const int Err_InvalidOnOff = -3;    // on_off 取值非法（只允许 0/1）
        private const int Err_Unexpected = -9;      // 兜底：未预期异常

        /// <summary>防呆：校验卡号/轴号并生成存储键。键用 long 防止 int 溢出；非法输入返回 false，不抛异常。</summary>
        private static bool TrySevonKey(int CardNo, int axis, out long key)
        {
            key = 0;
            if (CardNo < 0 || axis < 0) return false;
            key = (long)CardNo * 1000000L + axis;
            return true;
        }
        /// <summary>
        /// 控制指定轴的伺服使能端口的输出
        ...
        public int CardAxisWriteSevonPin(int CardNo, int axis, int on_off)
        {
            ...
            // 卡号不能小于0
            if (CardNo < 0)
                return Err_InvalidCardNo;

            // 轴号不能小于0
            if (axis < 0)
                return Err_InvalidAxis;

            // on_off只允许0/1
            if (on_off != 0 && on_off != 1)
                return Err_InvalidOnOff;

            // ===== 防呆校验 end =====

            try
            {
                if (!TrySevonKey(CardNo, axis, out var key))
                    return Err_InvalidCardNo;   // 非法卡号/轴号：忽略，绝不抛异常

                // 直接写入（首次写即自动登记该轴）。注意：**不能**要求「key 必须预先存在」，
                // 否则「第一次点某根轴的使能」会因 key 不存在而被拒绝，表现为按钮点了没反应。
                _sevonByAxis[key] = on_off;
                return OK;
            }
            catch
            {
                // 兜底：任何未预期异常都不向外抛，避免拖垮调用方 / 内核后台线程。
                return Err_Unexpected;
            }
            //dmc_write_sevon_pin((ushort)CardNo, (ushort)axis, (ushort)on_off);
        }
        ...
        {    
            // ===== 防呆：非法卡号/轴号按「失能」处理（返回 1），绝不抛异常 =====
            if (CardNo < 0 || axis < 0) return 1;
            try
            {
                if (!TrySevonKey(CardNo, axis, out var key)) return 1;
                // 从未写过时返回 1（失能）：与真实硬件一致 —— 新轴上电默认不使能，需用户手动点「使能」。
                return _sevonByAxis.TryGetValue(key, out var v) ? (v == 0 ? 0 : 1) : 1;
            }
            catch { return 1; }   // 兜底：读取失败一律按「失能」返回，绝不向外抛
            //dmc_read_sevon_pin((ushort)CardNo, (ushort)axis);
        }
        ...
        public uint GetCardOutPort(int CardNo, int portno)
        {
            ...
            for (ushort i = 0; i <= 31; i++)
            {
                if(MainBordOutputIOBitBuffer!=null &&i >= MainBordOutputIOBitBuffer.Count)
                {
                    continue;
                }
        ...
        { 
            for (int i = 0; i < 32; i++)
            {
                MainBordOutputIOBitBuffer[(ushort)i] = new VitualCardOutputIOStruct
                {
                    Name = $"Output_{i}",
                    OutputPortNo = i,
                    BitValue = ((outport_val >> i) & 1) == 1
                }; 
            } 
        ...
        {
            return axisStatusParameter[axis][(int)AxisStatusParameterEnum.目标位置];
        ...
        public int GetCard_CANIO_OutPort(int CardNo, int NodeID, int PortNo, ref UInt32 IoValue)
        {
            ...
            if (PortNo == 0)
            {
                for (ushort i = 0; i <= 31; i++)
                {
                    if (ExtendOutputIOBitBuffer.ContainsKey($"{NodeID},{i}"))
                    {
        ...
        public int GetCard_CANIO_InPort(int CardNo, int NodeID, int PortNo, ref UInt32 IoValue)
        {
            ...
            if (PortNo == 0)
            {
                for (ushort i = 0; i <= 31; i++)
                {
                    if (ExtendInputIOBitBuffer.ContainsKey($"{NodeID},{i}"))
                    {
