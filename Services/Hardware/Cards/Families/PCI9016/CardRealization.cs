...
//   源: MotionCardRes/PCI9016/CardRealization.cs
...
namespace Samsun.Domain.MotionCard.Common.PCI9016
{
    public class CardRealization : ICard
    {
        ...
        /// </summary>
        public SystemSupportMotionCardEnum HardwareName { get; set; } = SystemSupportMotionCardEnum.PCI9016;
        ...
        public CardRealization()
        {
            for (int i = 0; i < 16; i++)
            {
                ListCardParam.Add(new CardParamModel
                {
                    CardNo = 0,
                    CardType = 0,
                    CardName = SystemSupportMotionCardEnum.PCI9016.ToString(),

                    CardRemarks = "PCI9016总线卡，16IO输出，16IO输出",
                    CardSuportAxisNum = 6,
        ...
        public int InitCard()
        {
            ...
            int[] card_id = null;
            int card_count = 0;
            var res = PCI9016SDK.Instance.InitCard(ref card_id, ref card_count);
            var errmsg = PCI9016SDK.Instance.GetErrorInfo(res);

            if (res != 0)
            {
                isInit = false;
                CMessage.Instance.AddExceptionMessage(DeviceMessageTypeEnum.报警消息窗口, $"初始化[{SystemSupportMotionCardEnum.PCI9014.ToString()}]卡错误:{errmsg} ", "InitCard()", Resource1.softbugMsg1, false);
                return -1;
            }
            cardID.Clear();
        ...
        public int OpenCard(ref ushort cardnum, ref uint[] cardtypes, ref ushort[] CardID)
        {
            ...
            PCI9016SDK.Instance.homeConfigArray = new HomeParameModel[4 * (cardID[cardCount - 1] + 1)];
            //PCI9014SDK.Instance.homeConfigArray = new HomeParameModel[4 * cardnum];
            //9014不支持OpenCard,所以这里直接赋值缓存的信息
            cardnum = (ushort)cardCount;
            List<ushort> list1 = new List<ushort>(); cardtypes = new uint[8];
            for (int i = 0; i < cardnum; i++)
            {
                if (this.HardwareName == SystemSupportMotionCardEnum.PCI9016) cardtypes[i] = 9016;
                list1.Add((ushort)cardID[i]);
            }
            CardID = list1.ToArray();
            isOpen = true;
            return 0;
            //if (this.HardwareName == SystemSupportMotionCardEnum.PCI9014)
            //{
            //    cardtypes = new uint[] { 9014, 0, 0, 0, 0, 0, 0, 0 };
            //}
            //cardnum = (ushort)cardCount;
        ...
        public int GetCardLibVersion(int CardID, ref int LibVer, ref int pDriver_ver, ref int pLogic_ver)
        {
            try
            {
                ...
                uint pdriverVer = 0;
                uint plogicver = 0;
                int result = PCI9016SDK.Instance.GetCardVersion(CardID, ref libVer, ref pdriverVer, ref plogicver);
        ...
        {
            if (ListCardParam.Count < 1)
            {
                TotalAxis = 0; return 1;
            }
        ...
        {
            return PCI9016SDK.Instance.GetErrorInfo(ErrNum);
        }
