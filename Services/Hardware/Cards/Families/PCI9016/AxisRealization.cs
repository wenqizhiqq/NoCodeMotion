...
//   源: MotionCardRes/PCI9016/AxisRealization.cs
//   改动: 仅行尾归一化为 LF、加 #nullable disable；命名空间与逻辑保持原样。
// ────────────────────────────────────────────────────────────────
#nullable disable

using Samsun.Domain.MotionCard.Common.PCI9014;
using System;
...
namespace Samsun.Domain.MotionCard.Common.PCI9016
{
    public class AxisRealization : IAxis
    {
        ...
        {
            return PCI9016SDK.Instance.SetCardAxisPos(AxisID, cntr_on, Pos);
        }
        ...
        public int AxisAlarm
        {
            ...
            {
                uint level = 0;
                var res = PCI9016SDK.Instance.GetCardDIBit(AxisWhichCardNo, (uint)AxisID, ref level);
                return 0; // (int)level;
            }
            set
            {
        ...
        {
            uint status = 0;
            var res = PCI9016SDK.Instance.GetAxisCardIOStatus(AxisID, ref status);
            if (res == 0)
                return (int)status;
            else
                return res;
        ...
        {
            var res = PCI9016SDK.Instance.GetErrorInfo(ErrNum);
            return res;
        }
        ...
        {
            return PCI9016SDK.Instance.SetCardAxisHomeConfig(hpm.Axis, hpm.HomeMode, hpm.OrgLevel, hpm.EZlevel);
        }
        ...
        {
            //SetCardAxisHomeProfile();已经设置过
            if (PCI9016SDK.Instance.homeConfigArray == null || PCI9016SDK.Instance.homeConfigArray.Count() < (4 * (hpm.CardNo + 1))) PCI9016SDK.Instance.homeConfigArray = new HomeParameModel[4 * (hpm.CardNo + 1)];
            PCI9016SDK.Instance.homeConfigArray[hpm.Axis] = hpm;
            return 0;
        }
        ...
        {
            return PCI9016SDK.Instance.SetCardAxisELLevel(lpm.Axis, lpm.ActiveLevel);
        }
        ...
        {
            return PCI9016SDK.Instance.SetCardAxisErrorStopMode(axis, stop_mode);
        }
        ...
        {
            return PCI9016SDK.Instance.SetCardAxisPos(axis, 0, pos);
        }
        ...
        public int GetCardAxisInitVel(/*int CardNo,int Axis, */ref int pSpeed)
        {
            ...
            double pspeed = 0;
            int result = PCI9016SDK.Instance.GetCardAxisCurrentSpeed(AxisID, ref pspeed);
            pSpeed = (int)pspeed;
            return result;
        }
        ...
        public double GetCardAxisCurrentPosition(int cntr_no/*, int Axis*/)
        {
            ...
            //所以写成下面这样
            if (PCI9016SDK.Instance.GetCardAxisPos(AxisID, cntr_no, ref pos) == 0)
            {
                return pos;
            }
            return -1;
        ...
        public int GetCardAxisCurrentState()
        {
            ...
            uint u = 0;
            int axis = AxisID;
            var res = PCI9016SDK.Instance.GetCardAxisMotionStatus(axis, ref u);
            if (res == 0)
                return (int)u;
            else
                return -1;
        ...
        {
            return PCI9016SDK.Instance.SetCardAxisAlarm(axis, enable, active_level);
        }
        ...
        {
            return PCI9016SDK.Instance.SetCarsAxisSProfile(mpm.Axis, mpm.MinVel, mpm.MaxVel, mpm.TaccVel, mpm.TdecVel, mpm.JerkPercent);
        }
        ...
        {
            return PCI9016SDK.Instance.SetCarsAxisTProfile(mpm.Axis, mpm.MinVel, mpm.MaxVel, mpm.TaccVel, mpm.TdecVel);
        }
        ...
        {
            //if (PCI9016SDK.Instance.homeConfigArray[AxisID].HomeMode != 2)
            //{

            //    PCI9016SDK.Instance.CardAxisHomeMove(AxisID, 0);
            //    PCI9016SDK.Instance.CardAxisPMove(AxisID, (int)PCI9016SDK.Instance.homeConfigArray[AxisID].HomeTrigNegMoveDir, 0, 1);
            //    SetCardAxisTProfile(new MotionParamModel
            //    {
            //        Axis = PCI9016SDK.Instance.homeConfigArray[AxisID].Axis,
            //        CardNo = PCI9016SDK.Instance.homeConfigArray[AxisID].CardNo,
            //        MinVel = 1000,
            //        MaxVel = 1500,
            //        TaccVel = 0.1,
            //        TdecVel = 0.1
            //    });
            //    return PCI9016SDK.Instance.CardAxisHomeMove(AxisID, 0);
            //}
            //else
            //    return PCI9016SDK.Instance.CardAxisHomeMove(AxisID, 0);

            if (PCI9016SDK.Instance.homeConfigArray[AxisID].HomeMode == 4 || PCI9016SDK.Instance.homeConfigArray[AxisID].HomeMode == 2)
            {

                PCI9016SDK.Instance.CardAxisHomeMove(AxisID, 0);
                PCI9016SDK.Instance.CardAxisPMove(AxisID, (int)PCI9016SDK.Instance.homeConfigArray[AxisID].HomeTrigNegMoveDir, 0, 1);
                /*Thread.Sleep(1000); *///二次回原，第一次回原后延时在执行一次回原
                uint UState = 0; var error2 = 0; uint check_done = 0;
                while (true)
                {
                    error2 = CPci9014.p9014_get_io_status(AxisID, ref UState);
                    CPci9014.p9014_get_motion_status(AxisID, ref check_done);
                    if ((UState >> 4) % 2 == 0 || CConfigurationInfo.Instance.ReadEstopSignalByHardwareTable())
                    {
                        CPci9014.p9014_stop(AxisID, 1); return 205;
                    }
                    if (check_done == 0)
                    {//轴停止运动
                        break;
        ...
        {
            return PCI9016SDK.Instance.CardAxisPMove(mpm.Axis, (int)mpm.Dist, mpm.PosiMode, mpm.RunMode);
        }
        ...
        {
            return PCI9016SDK.Instance.CardAxisVMove(mpm.Axis, mpm.Dir, mpm.RunMode);
        }
        ...
        {
            return PCI9016SDK.Instance.CardAxisStop(axis, emg_stop);
        }
        ...
        {
            return PCI9016SDK.Instance.SetCardAxisPos(axis, 1, encoder_value);
            //throw new NotImplementedException();
        }
        ...
        public int SetStatusParameter(int axis, bool f, AxisStatusParameterEnum kind)
        {
            ...
            if (kind.ToString() == "正限位" || kind.ToString() == "负限位")
            {
                if (f == true)
                {
                    res = PCI9016SDK.Instance.SetCardAxisELLevel(axis, 1);
                }
                else
                {
                    res = PCI9016SDK.Instance.SetCardAxisELLevel(axis, 0);
                }
            }
            if (kind.ToString() == "原点位")
            {
                if (f == true)
                {
                    res = PCI9016SDK.Instance.SetCardAxisHomeConfig(axis, 0, 1, 0);
                }
                else
                {
                    res = PCI9016SDK.Instance.SetCardAxisHomeConfig(axis, 0, 0, 0);
                }
            }
        ...
        public int CardAxisGetHomeResult(int CardNo, int axis, ref ushort status)
        {
            //throw new NotImplementedException();
            uint pstatus = 1;
            var res = PCI9016SDK.Instance.GetCardAxisMotionStatus(axis, ref pstatus);
            status = (ushort)pstatus;
            return res;
        }
        ...
        public double GetCardAxisCurrentSpeed(int CardNo, int axis)
        {
            ...
            var res = PCI9016SDK.Instance.GetCardAxisCurrentSpeed(axis, ref currentSpeed);
