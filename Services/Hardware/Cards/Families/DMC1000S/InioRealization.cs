...
//   源: MotionCardRes/DMC1000S/InioRealization.cs
...
namespace WenQiZhi.Domain.MotionCard.Common.DMC1000S
{
    public class InioRealization : IIOInPut
    {
        ...
        /// </summary>
        public int IOIndexStart { get; set; } = 1;


        /// <summary>
        /// 检测急停IO，如果触发则停止全部轴(此功能目前仅对卡dmc1000S有意义，因为此卡无硬件急停接入)
        /// </summary>
        /// <param name="CardName">要做急停动作的卡的名字</param>
        /// <param name="cardNo">卡号</param>
        /// <param name="EmgIO">急停IO的索引</param>
        /// <returns></returns>
        public int EMGAction(string CardName, int cardNo, int EmgIO)
        {
            if (InIOName.IndexOf(CardName) < 0) return 0;
            var data = DMC1000SSDK.Instance.GetCardInPort(cardNo, EmgIO);
            var s1 = Convert.ToString(data, 2).PadLeft(InIONums, '0');
            s1 = new string(s1.ToCharArray().Reverse().ToArray());
            for (int i = 0; i < InIONums; i++)
            {
                if (i == EmgIO)
                {
                    var iostatus = (ushort)(s1[i] == '1' ? 1 : 0);
                    if (iostatus == 1)
                    {
                        if (DMC1000SSDK.Instance.GetCardAxisCheckDone(cardNo, 0) == 0)
                        {
                            DMC1000SSDK.Instance.CardAxisStop(cardNo, 0, 1);
                        }
                        if (DMC1000SSDK.Instance.GetCardAxisCheckDone(cardNo, 1) == 0)
                        {
                            DMC1000SSDK.Instance.CardAxisStop(cardNo, 1, 1);
                        }
                        if (DMC1000SSDK.Instance.GetCardAxisCheckDone(cardNo, 2) == 0)
                        {
                            DMC1000SSDK.Instance.CardAxisStop(cardNo, 2, 1);
                        }
                        if (DMC1000SSDK.Instance.GetCardAxisCheckDone(cardNo, 3) == 0)
                        {
                            DMC1000SSDK.Instance.CardAxisStop(cardNo, 3, 1);
                        }
                        break;
                    }
                }
        ...
        {
            return DMC1000SSDK.Instance.SetCardWriteInBit(CardNo, bitno, on_off);
        }
        ...
        {
            return (int)DMC1000SSDK.Instance.GetCardInPort(CardNo, portno);
        }
        ...
        {
            uint IoValue1 = 0;
            DMC1000SSDK.Instance.GetCard_CANIO_InPort(CardNo, NodeID, PortNo, ref IoValue1);
            return IoValue1;
        }
        ...
        {
            //int error = DMC_E3032SDK.Instance.NmcReadOutbitExtern(CardNo, Channel, NoteID, IoBit, ref IoValue);
            //return IoValue;
            return -1;
        }
    }
