...
// 提供：工件动态刚体（重力 + 被整个机台接住）、床身/底脚/相机/转台/气缸缸体等静态碰撞体、
// 龙门/滑座/主轴/活塞杆等运动学碰撞体（由流程运行时驱动）的增删与步进，以及工件拾取/拖拽位姿读写。
// 所有机台部件均参与碰撞——工件被整个机台兜住，并设兜底地面防止无限下落；运动学碰撞体每帧跟随
// 流程位置移动，从而物理推动工件——实现「仿真与流程交互」。
using System;
