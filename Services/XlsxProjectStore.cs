...
namespace NoCodeMotion.Services
{
    ...
    public static class XlsxProjectStore
    {
        ...
        /// <summary>把点位的移动条件编码为 xlsx 文本：多条 ';' 分隔，每条「类型|目标名|比较|期望状态|启用」；名称留空的行不落盘。</summary>
        private static string EncodeConditions(object pointObj)
        {
            ...
            foreach (var cObj in conds)
            {
                ...
                // 「使用」勾选位：勾选存 1、未勾选存 0（未勾选但已填名称的行同样落盘，避免用户配置丢失）
                bool used = t.GetProperty("IsUsed")?.GetValue(cObj) as bool? ?? true;
                if (sb.Length > 0) sb.Append(';');
                sb.Append(kind).Append('|').Append(target).Append('|').Append(cmp).Append('|')
                  .Append(state).Append('|').Append(used ? "1" : "0");
            }
            return sb.ToString();
        }

        /// <summary>解析 xlsx「条件」单元格并回填到点位的 Conditions 集合，最后规整为固定 6 行。
        /// 兼容三种格式：旧「类型|目标名|期望状态」（3 段，比较默认 ==、默认启用）、
        /// 「类型|目标名|比较|期望状态」（4 段，默认启用）与新「类型|目标名|比较|期望状态|启用」（5 段）。</summary>
        private static void LoadConditions(PointItem point, string? text)
        {
            if (!string.IsNullOrWhiteSpace(text))
            {
                foreach (var seg in text.Split(';', System.StringSplitOptions.RemoveEmptyEntries))
                {
                    ...
                    bool hasComparison = parts.Length >= 4;   // 4 段起才有比较列；3 段是旧工程
                    bool hasUsed = parts.Length >= 5;         // 5 段起才有启用位
                    point.Conditions.Add(new PointMoveCondition
                    {
                        ...
                        IsUsed = !hasUsed || parts[4] != "0",   // 旧格式没有这一位 → 默认启用
                    });
                }
            }
